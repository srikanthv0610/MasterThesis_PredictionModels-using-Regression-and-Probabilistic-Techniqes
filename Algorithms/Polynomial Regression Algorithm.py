...
def main():
    ...
    def poly_regressor(start_len, end_len, train_len, predict_len):
        ...
        for i in range(start_len, end_len, 1):
            ...
            thetaX = np.array([[float(t0), 0, 0]])
            thetaY = np.array([[float(t1), 0, 0]])
            ...
            LeftX_predict = theta_LeftX[:, 0] + (
                    (theta_LeftX[:, 1]) * Time[i + train_len: i + train_len + predict_len]) + \
                            ((theta_LeftX[:, 2]) * (Time[i + train_len: i + train_len + predict_len]) ** 2)
            LeftY_predict = theta_LeftY[:, 0] + (
                    (theta_LeftY[:, 1]) * Time[i + train_len: i + train_len + predict_len]) \
                            + ((theta_LeftY[:, 2]) * (Time[i + train_len: i + train_len + predict_len]) ** 2)
    ...
    def computeCost(X, y, theta):
        return np.sum(((X @ theta.T) - y)**2)/ (2 * len(X))
    ...
    poly_regressor(start_len=0000, end_len=50000, train_len=5, predict_len=20)
